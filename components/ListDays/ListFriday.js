...
export default class ListFriday extends Component {
  constructor(props) {
    super(props)
...
getEvents = async() => {
    ...
    const API_KEY = key;
    const beginDate = moment().clone().tz("Asia/Jerusalem").startOf('week').add(5, 'days');
    beginDate.set({hour:0,minute:0,second:0,millisecond:0});
    const finishDate= moment().clone().tz("Asia/Jerusalem").startOf('week').add(6, 'days');
