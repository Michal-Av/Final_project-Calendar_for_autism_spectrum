...
export default class ListSunday extends Component {
  constructor(props) {
    super(props)
...
getEvents = async() => {
  ...
  const API_KEY = key;
    const beginDate = moment().clone().tz("Asia/Jerusalem").startOf('week');
    beginDate.set({hour:0,minute:0,second:0,millisecond:0});
    const finishDate= moment().clone().tz("Asia/Jerusalem").startOf('week').add(1, 'days');
